...
// Registration writes to the LongLive container's volume, which can be mid cold start.
...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(
      new URL("/personas/register", env.LONGLIVE_URL.replace(/^ws/, "http")),
      {
        method: "POST",
    ...
    console.info(
      `live/personaRegister: user=${registrar.uuid} sha256=${sha256}`,
    );
    return NextResponse.json({
