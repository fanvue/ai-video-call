...
const request = (body: unknown) =>
  new Request("https://app.test/api/live/personaRegister", {
    method: "POST",
...
describe("POST /api/live/personaRegister", () => {
  ...
  it("registers under the hash id, sends a purpose-bound token and logs only the user id and hash", async () => {
    ...
    expect(url.toString()).toBe(
      "https://example-longlive.modal.run/personas/register",
    );
    ...
    expect(info).toHaveBeenCalledWith(
      `live/personaRegister: user=user-uuid-1 sha256=${SHA}`,
    );
    info.mockRestore();
  });
