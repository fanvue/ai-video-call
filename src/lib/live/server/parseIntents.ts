...
- {"type":"act","act":"twerk"|"grind"|"bounce"|"spread"|"sway"|"crawl"|"spin"|"gesture"|"tongue"|"tease"|"dance"|"doggy"|"spank"|"boobPlay"}  (gesture = wave, wink, blow a kiss, smile for the camera)
...
Use the catalogue types whenever one fits; verbatim only for real physical actions outside it. Never invent a step the viewer did not ask for. "strip" or "take it all off" is one removeGarment per garment she still wears.`;
...
export const parseIntentsWithLlm = async (
  ...
  try {
    ...
    const intents = validIntents(
      JSON.parse(raw.match(/\{[\s\S]*\}/)?.[0] ?? raw),
    );
