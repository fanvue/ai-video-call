...
describe("generateClip: hold clips other than idle", () => {
  it("greeting off a raw upload chains like any other hold clip: midpoint and last frame checked, last frame becomes the seed", async () => {
    renderBackendFor.mockReturnValue({ supportsEndFrame: true, render });
    guardFrame.mockResolvedValue({
      checked: true,
      issues: [],
    ...
    const req = clipRequest({
      job: { kind: "greeting" },
      session: { ...raw, seedFrameUrl: raw.anchorFrameUrl },
    });
