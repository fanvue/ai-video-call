...
describe("createEarlySwaps", () => {
  it("finalizeSwap reuses the swap started at render instead of starting a second one", async () => {
    ...
    const taken = early.take({ videoUrl: "https://example.com/raw.mp4", jobKind: "reply" });
    pending.resolve("swapped");

    await expect(taken).resolves.toBe("swapped");
    expect(runSwap).toHaveBeenCalledTimes(1);
    expect(runSwap).toHaveBeenCalledWith({ videoUrl: "https://example.com/raw.mp4", jobKind: "reply" });
  });
  ...
  it("a clip with no early swap gets nothing, and one is only taken once", () => {
    ...
    expect(early.take({ videoUrl: "https://example.com/b.mp4", jobKind: "idle" })).toBeUndefined();
    expect(early.take({ videoUrl: "https://example.com/a.mp4", jobKind: "reply" })).toBeDefined();
    expect(early.take({ videoUrl: "https://example.com/a.mp4", jobKind: "reply" })).toBeUndefined();
  });
