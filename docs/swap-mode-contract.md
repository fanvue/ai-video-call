...
- Capacity: `max_containers=4`, `buffer_containers=2`, `scaledown_window=600`. A prod session showed `serviceMs` of 14 to 21 s against `swap_ms` of 6 s: the fourth concurrent clip queued behind a ~15 s cold start, the beat and bridge idles were late, and the player fell back to cuts. Two warm spares absorb that; the log line carries `downloadMs` so a slow fal fetch is told apart from queueing. The next session then measured a 93 s cold boot (image pull; the models load in 5 s) with the autoscaler never adding a second container, so the greeting swap hit its budget, the join took 54 s, the greeting ended on a 50 s hold and the first reply took 47 s. Now `min_containers=1` keeps one container warm around the clock (about one L40S hour of spend per idle hour, the lever to turn off when the spike is parked) and `@modal.concurrent(max_inputs=3)` lets that container take the join burst at once rather than one clip at a time. `SWAP_GREETING_BUDGET_MS` dropped to 20 s: past a warm swap the container is cold, and the reference-to-video greeting already carries the identity, so playing it unswapped is the faster join.
