...
def _check_entry(root: str, entry: object) -> tuple[Persona | None, str]:
    ...
    note = entry.get("note")
    return Persona(persona_id, path, note[:MAX_NOTE_CHARS] if isinstance(note, str) else ""), ""
...
def list_personas(root: str) -> list[dict]:
    """Only what the picker needs: id and note of the entries that would resolve."""
    try:
        entries = _read_manifest(root)
    ...
    for entry in entries:
        ...
        if persona is not None:
            listed.append({"id": persona.id, "note": persona.note})
    return listed


def registered_entry(sha256: str, extension: str, added_by: str, added_at: str) -> dict:
